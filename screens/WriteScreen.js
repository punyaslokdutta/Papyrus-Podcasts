...
import { Image, StyleSheet,SafeAreaView, ScrollView, TextInput,Alert, TouchableOpacity , View,ActivityIndicator, Linking,Dimensions,NativeModules} from 'react-native'
...
const height1 =ExtraDimensions.getRealWindowHeight();
...
class WriteScreen extends Component {
  ...
  constructor(props)
  {
   super(props)
   {
    ...
    handleScroll = (event) => {
      console.log("[WriteScreen - handleScroll] event.nativeEvent.contentOffset.y: ",event.nativeEvent.contentOffset.y);
      console.log("[WriteScreen - handleScroll] event.nativeEvent.contentSize.height: ",event.nativeEvent.contentSize.height);
      console.log("[WriteScreen - handleScroll] Dimensions.getWINDOW: ",event.nativeEvent.layoutMeasurement.height);
      this.setState({
        percent : event.nativeEvent.contentOffset.y / (event.nativeEvent.contentSize.height - event.nativeEvent.layoutMeasurement.height)
      })
    };
