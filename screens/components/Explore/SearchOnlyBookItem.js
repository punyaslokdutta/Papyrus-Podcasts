...
const styles = StyleSheet.create({
 ...
 const areEqual = (prevProps, nextProps) => true
 const SearchOnlyBookItem = React.memo((props)=> {
  console.log("Inside SearchOnlyBookItem");
  const dispatch = useDispatch();
  console.log(props);

  async function retrieveBookData(){
    const onlyBookDoc = await firestore().collection('books').doc(props.book.objectID).get();
    dispatch({type:"SELECT_ONLY_BOOK_ITEM",payload:onlyBookDoc._data})
  }
        ...
        return (
          <TouchableOpacity onPress={() => {
            ...
            props.navigation.navigate("SearchChapterScreen");
            //props.navigation.navigate('RecordBook', {book : props.book.objectID })
             ...
             <View style={{flexDirection: 'row', justifyContent: 'flex-end',paddingTop:height/48,paddingLeft:width/8}}>
              <Image style={{width:width/4,height:height/8}} source={ {uri: props.book.bookPictures}} />
            </View>
...
export default SearchOnlyBookItem;